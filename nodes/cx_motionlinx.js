...
module.exports = function (RED) {
    ...
    let currentMasterState = "MASTER STOPPED";
    ...
    function MasterConfig(config) {
        ...
        config.slaveQty = Number.parseInt(config.slaveQty);
        config.frequencyHz = Number.parseInt(config.frequencyHz);
        ...
        const slaveConfig = Array(config.slaveQty).fill(0).map((v, i) => {
            const conf = JSON.parse(JSON.stringify(tools_1.motionLinxConfig));
            conf.position = i;
            return conf;
        });
        ...
        if (ecatMaster.isFake)
            currentMasterState = "FAKE MASTER";
    ...
    function DeviceStatus(config) {
        ...
        node.on("input", (msg) => {
            ...
            if (msg.topic === "__ethercat_status__" && msg.request && typeof msg.request === "string") {
                (0, child_process_1.exec)("ethercat " + msg.request, (err, stdout, stderr) => {
                    if (err || stderr) {
